...
public:
	...
	void begin(), boldOff(), boldOn(), doubleHeightOff(), doubleHeightOn(),
			doubleWidthOff(), doubleWidthOn(), feed(uint8_t x = 1), feedRows(
					uint8_t), flush(), inverseOff(), inverseOn(), justify(
					char value), offline(), online(), printBarcode(
					const char *text, uint8_t type), printBitmap(int w, int h,
					const uint8_t *bitmap, int density = 1), printBitmap_P(
					int w, int h, const uint8_t *bitmap, int density = 1),
			printBitmap(int w, int h, const uint8_t *bitmap, bool fromProgMem =
					true), normal(), reset(), setBarcodeHeight(
					uint8_t val = 50), setCharSpacing(int spacing = 0),
			setCharset(uint8_t val = 0), setCodePage(uint8_t val = 0),
			setDefault(), setLineHeight(int val = 30), setMaxChunkHeight(
					int val = 256), setSize(char value), setSize(uint8_t height,
					uint8_t width), setTimes(unsigned long, unsigned long),
			sleep(), sleepAfter(uint16_t seconds), strikeOff(), strikeOn(),
			tab(), test(), testPage(), underlineOff(), underlineOn(
					uint8_t weight = 1), upsideDownOff(), upsideDownOn(),
			wake();
	...
	void appendBytes(uint8_t a);
	void newLine();

private:

	uint8_t printMode, prevByte,      // Last character issued to printer
			...
			maxChunkHeight;
	void writeBytes(uint8_t a), writeBytes(uint8_t a, uint8_t b), writeBytes(
			uint8_t a, uint8_t b, uint8_t c), writeBytes(uint8_t a, uint8_t b,
			uint8_t c, uint8_t d), setPrintMode(uint8_t mask), unsetPrintMode(
			uint8_t mask), writePrintMode();
